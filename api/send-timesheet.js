...
async function generatePDF(data) {
  return new Promise((resolve, reject) => {
    ...
    // ── Table ──
    const cols   = [40, 88, 142, 192, 238, 284, 330, 390, 462];
    const heads  = ['Day','Type','Date','Start','End','Break','Hours','Client / Unit','Auth'];
    const tTop   = 134;
    ...
    data.days.forEach(dayData => {
      ...
      const drawRow = (rowData, label, isShift) => {
        if (!rowData) return;
        doc.rect(40, y - 2, W - 80, 16)
           .fill(isShift ? (alt ? '#f8f8f8' : 'white') : '#f0f8ff');

        doc.fillColor('#1b3a5c').font('Helvetica-Bold').fontSize(8)
           .text(isShift ? day.slice(0,3) : (shift ? '' : day.slice(0,3)), cols[0]+2, y+2);

        doc.fillColor(isShift ? '#1a6b2a' : '#094f80')
           .font('Helvetica').fontSize(8).text(label, cols[1]+2, y+2);

        doc.fillColor('#1b3a5c').font('Helvetica').fontSize(8)
           .text(rowData.date||'-',  cols[2]+2, y+2)
           .text(rowData.start||'-', cols[3]+2, y+2)
           .text(rowData.end||'-',   cols[4]+2, y+2)
           .text(rowData.brk > 0 ? `${(rowData.brk/60).toFixed(2)}` : '-', cols[5]+2, y+2);

        doc.fillColor(isShift ? '#1a6b2a' : '#0e8fd4')
           .font('Helvetica-Bold').fontSize(8).text(rowData.hours, cols[6]+2, y+2);

        const clientUnit = `${rowData.client||'-'}${rowData.unit ? ' / '+rowData.unit : ''}`;
        doc.fillColor('#1b3a5c').font('Helvetica').fontSize(7)
           .text(clientUnit.substring(0,18), cols[7]+2, y+2)
           .text((rowData.auth||'-').substring(0,10), cols[8]+2, y+2);
    ...
    // ── Signatures ──
    const sigDays = data.days.filter(d =>
      (d.shift && d.shift.sig) || (d.sleepIn && d.sleepIn.sig)
    );
    if (sigDays.length > 0) {
      y += 35;
      doc.fillColor('#1b3a5c').font('Helvetica-Bold').fontSize(9)
         .text('CLIENT SIGNATURES — TURNER HOME', 40, y);
      y += 14;
      sigDays.forEach(dayData => {
        const drawSig = (rowData, label) => {
          if (!rowData?.sig) return;
          doc.fillColor('#1b3a5c').font('Helvetica').fontSize(8)
             .text(`${dayData.day} · ${label} · Authorised by: ${rowData.auth||'N/A'}`, 40, y);
          try {
            const buf = Buffer.from(rowData.sig.split(',')[1], 'base64');
            doc.image(buf, 320, y-4, { height: 28 });
          } catch(e) { /* skip if sig fails */ }
          y += 32;
        };
        drawSig(dayData.shift,   'Shift');
        drawSig(dayData.sleepIn, 'Sleep In');
      });
    }
