...
export default function App() {
  ...
  // ── Auto-save: restore from device on load ──────────
  useEffect(()=>{
    try {
      const saved = localStorage.getItem("nexime_draft");
      if (saved) {
  ...
  // ── Auto-save: write to device on every change ──────
  useEffect(()=>{
    if (step === "done") return; // don't save done state
    try {
      localStorage.setItem("nexime_draft", JSON.stringify({ info, rows, step }));
    } catch(e){}
  },[info, rows, step]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setSendStatus("sent");
        // Clear saved draft — timesheet submitted successfully
    ...
    <div style={{minHeight:"100vh",background:"#f4faff",fontFamily:"'Nunito','Segoe UI',sans-serif",color:NAVY}}>
      ...
      <div style={{maxWidth:580,margin:"0 auto",padding:"24px 16px 60px"}}>
        ...
        {step==="shifts"&&(
          <div>
            {/* Auto-save notice */}
            <div style={{background:"#e8faf0",border:`1px solid ${GREEN}44`,borderRadius:8,
              padding:"7px 12px",marginBottom:10,display:"flex",alignItems:"center",gap:8,fontSize:11}}>
              <span>💾</span>
              <span style={{color:"#1a6b2a",fontWeight:600}}>Auto-saving — your progress is saved on this device</span>
            </div>
